...
userids=`mysql -N -h localhost -u $MYSQLUSER $MYSQLDB -e "SELECT DISTINCT(USERID) from PROXYMASTER where DATE(edate) <= curdate() and TIME(etime) < curtime() ;"`
echo "Proxy Expiry Log: $(date +%T)" >> /var/tmp/proxydelete.log
if [ -z $userids ];then exit 0;fi
