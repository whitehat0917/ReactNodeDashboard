...
Menu_3()
{
	...
	do
		...
		then
			...
			echo "Enter all details correctly"
		fi
	done
    ...
    if [ $USERCOUNT -eq 1 ];then
        ...
        mysql -h localhost -u $MYSQLUSER  $MYSQLDB -e "INSERT INTO USERMASTER (username,password,type) VALUES ('$username','$password','M');"
...
Menu_19()
{
    ...
    if [ $COUNTOFIP -lt $RNDNO ]; then echo "Not enough available proxies to serve";exit 35;fi
        ...
        do
                ...
                LISTOFIPID=`mysql -N -h localhost -u $MYSQLUSER  $MYSQLDB -e "SELECT IPID FROM IPMASTER WHERE STATUS IS FALSE ORDER BY USED LIMIT 1;"`
                SDATE=`echo $(date +%F)`
                STIME=`echo $(date +%T)`
                EDATE=`echo $(date +%F -d "+$PXYDAYS days")`
                ETIME=`echo $(date +%T)`
                ...
                echo "Created proxy for User: $RNDUSER"
                Q=$((Q+1))
        done
    systemctl reload squid
}
...
Menu_21()
{
        LISTOFUSERS=`mysql -N -h localhost -u $MYSQLUSER  $MYSQLDB -e "SELECT USERNAME FROM USERMASTER WHERE TYPE='R';"`
