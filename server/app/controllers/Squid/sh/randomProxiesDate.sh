...
{	
        ...
        LISTOFIPID=`mysql -N -h localhost -u $MYSQLUSER  $MYSQLDB -e "SELECT IPID FROM TEMP_PROXYMASTER A LEFT JOIN PROXYMASTER B ON A.PXYID = B.PXYID WHERE A.GROUPID = $GROUPID;"`
